...
const main = (admin = false) => { const keyboard = new Keyboard().text("📅 Записаться").text("💰 Прайс").row().text("✂️ Услуги").text("🐶 Мои питомцы").row().text("📋 Мои записи").text("📍 Где мы находимся").row().text("💬 Связаться с администратором"); if (admin) keyboard.row().text("⚙️ Админ-панель"); return keyboard.resized(); };
...
bot.command(["start", "menu", "help"], async ctx => { const u = await current(ctx), cfg = await settings(); await clearFlow(u.id); await ctx.reply(ctx.match === "help" ? "Нажимайте кнопки меню — я помогу оформить запись." : cfg.welcomeText, { reply_markup: main(isOwner(ctx)) }); });
bot.hears("📅 Записаться", async ctx => { const u = await current(ctx); await clearFlow(u.id); await choosePet(ctx, u.id); });
...
bot.callbackQuery(/^b:(.+)$/, async ctx => { const u=await current(ctx), [kind,...rest]=ctx.match[1].split(":"); const value=rest.join(":"); const row=await prisma.botFlow.findUnique({where:{userId:u.id}}); const f=(row?.payload??{}) as Flow; await ctx.answerCallbackQuery();
  ...
  if(kind==="petadd"){await setFlow(u.id,"pet_name");await ctx.reply("Как зовут питомца? Напишите имя:");return;}
  if(kind==="pe"){await setFlow(u.id,"pet_name",{editPetId:value});await ctx.reply("Введите новое имя питомца:");return;}
  if(kind==="species"){await setFlow(u.id,"pet_breed",{...f,species:value});await ctx.reply("Укажите породу (или отправьте «—»):");return;}
...
bot.on("message:text", async ctx=>{ const u=await current(ctx), row=await prisma.botFlow.findUnique({where:{userId:u.id}}); if(!row)return; const f=row.payload as Flow, text=ctx.message.text.trim(); if(row.state==="pet_name"){await setFlow(u.id,"pet_species",{...f, name:text});await ctx.reply("Выберите вид животного:",{reply_markup:new InlineKeyboard().text("🐶 Собака","b:species:dog").text("🐱 Кошка","b:species:cat")});return;} if(row.state==="pet_breed"){const petData={name:f.name!,species:f.species!,breed:text==="—"?null:text};if(f.editPetId)await prisma.pet.update({where:{id:f.editPetId},data:{name:petData.name}});else await prisma.pet.create({data:{ownerId:u.id,...petData}});await clearFlow(u.id);await ctx.reply("🐾 Питомец сохранён.",{reply_markup:main(isOwner(ctx))});} });
...
if (require.main === module) void startPolling();
