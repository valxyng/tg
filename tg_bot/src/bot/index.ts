...
const main = (admin = false) => { 
  const keyboard = new Keyboard()
    .text("📅 Записаться").text("💰 Прайс").row()
    .text("✂️ Услуги").text("🐾 Мои питомцы").row()
    .text("👤 Мой профиль").text("📋 Мои записи").row()
    .text("📍 Где мы находимся").text("💬 Связаться с администратором");
  if (admin) keyboard.row().text("⚙️ Админ-панель");
...
async function current(ctx: Context) {
  ...
  return prisma.user.upsert({
    ...
    update: { username: ctx.from.username, firstName: ctx.from.first_name, lastName: ctx.from.last_name },
    create: { telegramId: BigInt(ctx.from.id), username: ctx.from.username, firstName: ctx.from.first_name, lastName: ctx.from.last_name }
  });
}
...
async function promptRegistration(ctx: Context) {
  await ctx.reply(
    "🐾 <b>Добро пожаловать в «Лапуню»!</b>\n\nСоздайте профиль, чтобы сохранять ваши данные, питомцев и историю записей.",
    { parse_mode: "HTML", reply_markup: new InlineKeyboard().text("✨ Создать профиль", "b:register") }
  );
}
...
async function profile(ctx: Context, userId: string) {
  const u = await prisma.user.findUniqueOrThrow({
    where: { id: userId },
    include: { _count: { select: { pets: { where: { isArchived: false } }, bookings: true } } }
  });
  await ctx.reply(
    `👤 <b>МОЙ ПРОФИЛЬ</b>\n\nИмя: ${u.firstName || "—"}\n📱 Телефон: ${u.phone || "—"}\n\n🐾 Питомцев: ${u._count.pets}\n📅 Записей: ${u._count.bookings}`,
    {
      parse_mode: "HTML",
      reply_markup: new InlineKeyboard()
        .text("🐾 Мои питомцы", "b:pets").text("📅 Мои записи", "b:bookings").row()
        .text("✏️ Изменить данные", "b:profileedit").row()
        .text("◀️ Назад", "b:menu")
    }
  );
}
...
async function petsList(ctx: Context, userId: string) {
  ...
  pets.forEach(p => kb.text(`${speciesLabel(p.species)} ${p.name}${p.breed ? ` — ${p.breed}` : ""}`, `b:petcard:${p.id}`).row());
...
async function petCard(ctx: Context, userId: string, petId: string) {
  const pet = await prisma.pet.findFirst({
    where: { id: petId, ownerId: userId, isArchived: false },
    include: { owner: true, bookings: { include: { service: true }, orderBy: { startsAt: "desc" }, take: 10 } }
  });
  ...
  await ctx.reply(
    `${speciesLabel(pet.species)} <b>${pet.name.toUpperCase()}</b>\n\nПорода: ${pet.breed || "Не указана"}\nВладелец: ${pet.owner.firstName || "—"}\n\n📋 История записей:\n${history}`,
    {
      parse_mode: "HTML",
      reply_markup: new InlineKeyboard()
        .text("📅 Записать на груминг", `b:bookpet:${pet.id}`).row()
        .text("✏️ Изменить", `b:pe:${pet.id}`).text("🗑 Удалить", `b:petremove:${pet.id}`).row()
        .text("◀️ Назад", "b:pets")
    }
  );
}
...
bot.command(["start", "menu", "help"], async ctx => {
  const u = await current(ctx), cfg = await settings();
  await clearFlow(u.id);
  if (!u.profileCompleted) return void promptRegistration(ctx);
  await ctx.reply(ctx.match === "help" ? "Нажимайте кнопки меню — я помогу оформить запись." : cfg.welcomeText, { reply_markup: main(isOwner(ctx)) });
});
...
bot.hears("📅 Записаться", async ctx => {
  ...
  if (!u.profileCompleted) return void promptRegistration(ctx);
  await clearFlow(u.id);
  await choosePet(ctx, u.id);
});

bot.hears("💰 Прайс", async ctx => {
  await current(ctx);
  const cats = await prisma.serviceCategory.findMany({ where: { isArchived: false }, include: { services: { where: { isActive: true, isArchived: false }, orderBy: { sortOrder: "asc" } } }, orderBy: { sortOrder: "asc" } });
  await ctx.reply(cats.map(c => `<b>${c.name}</b>\n${c.services.map(s => `• ${s.name} — ${money(s.priceKopecks)}`).join("\n")}`).join("\n\n") || "Прайс пока заполняется.", { parse_mode: "HTML" });
});

bot.hears("✂️ Услуги", async ctx => {
  await current(ctx);
  const list = await prisma.service.findMany({ where: { isActive: true, isArchived: false }, orderBy: { sortOrder: "asc" } });
  await ctx.reply(list.map(s => `✂️ <b>${s.name}</b>\n${s.description || ""}\n⏱ ${s.durationMinutes} мин. · 💰 ${money(s.priceKopecks)}`).join("\n\n") || "Услуги пока заполняются.", { parse_mode: "HTML" });
});

bot.hears(["🐶 Мои питомцы", "🐾 Мои питомцы"], async ctx => {
  const u = await current(ctx);
  if (!u.profileCompleted) return void promptRegistration(ctx);
  await petsList(ctx, u.id);
});

bot.hears("👤 Мой профиль", async ctx => {
  const u = await current(ctx);
  if (!u.profileCompleted) return void promptRegistration(ctx);
  await profile(ctx, u.id);
});

bot.hears("📋 Мои записи", async ctx => {
  const u = await current(ctx);
  if (!u.profileCompleted) return void promptRegistration(ctx);
  const rows = await prisma.booking.findMany({ where: { clientId: u.id, startsAt: { gte: new Date() }, status: { in: ["PENDING", "CONFIRMED"] } }, include: { pet: true, service: true, master: true }, orderBy: { startsAt: "asc" } });
  const kb = new InlineKeyboard();
  rows.forEach(b => kb.text(`❌ Отменить ${b.pet.name} · ${b.startsAt.toLocaleDateString("ru-RU")}`, `b:x:${b.id}`).row());
  await ctx.reply(rows.length ? rows.map(b => `${speciesLabel(b.pet.species)} ${b.pet.name}\n✂️ ${b.service.name}\n👩 ${b.master.name}\n📅 ${b.startsAt.toLocaleString("ru-RU")}`).join("\n\n") : "Предстоящих записей нет.", { reply_markup: kb });
});

bot.hears("📍 Где мы находимся", async ctx => {
  const c = await settings();
  const kb = new InlineKeyboard();
  if (c.mapUrl) kb.url("🗺 Открыть карту", c.mapUrl);
  await ctx.reply(`📍 ${c.name}\n${c.address}\n${c.directions || ""}`, { reply_markup: kb });
});

bot.hears("💬 Связаться с администратором", async ctx => {
  const c = await settings();
  await ctx.reply(c.adminUsername ? `Напишите администратору: @${c.adminUsername.replace("@", "")}` : c.phone ? `Телефон: ${c.phone}` : "Контакты скоро появятся.");
});

bot.callbackQuery(/^b:(.+)$/, async ctx => {
  const u = await current(ctx), [kind, ...rest] = ctx.match[1].split(":"); const value = rest.join(":");
  const row = await prisma.botFlow.findUnique({ where: { userId: u.id } });
  const f = (row?.payload ?? {}) as Flow;
  ...
  if (kind === "bookings") {
    const rows = await prisma.booking.findMany({ where: { clientId: u.id }, include: { pet: true, service: true }, orderBy: { startsAt: "desc" }, take: 20 });
    await ctx.reply(
      rows.length ? rows.map(b => `${b.startsAt.toLocaleDateString("ru-RU")} — ${b.pet.name} — ${b.service.name}`).join("\n") : "Записей пока нет.",
      { reply_markup: new InlineKeyboard().text("◀️ Назад", "b:profile") }
    );
    return;
  }
  ...
  if (kind === "bookpet") {
    const pet = await prisma.pet.findFirst({ where: { id: value, ownerId: u.id, isArchived: false } });
    if (!pet) return;
    await chooseCategory(ctx, u.id, { petId: pet.id });
    return;
  }
  if (kind === "petremove") {
    const pet = await prisma.pet.findFirst({ where: { id: value, ownerId: u.id, isArchived: false } });
    if (!pet) return;
    await ctx.reply("⚠️ Удалить питомца?\nИстория его записей сохранится.", { reply_markup: new InlineKeyboard().text("❌ Отмена", "b:pets").text("✅ Удалить", `b:petdelete:${value}`) });
    return;
  }
  if (kind === "petdelete") {
    const pet = await prisma.pet.findFirst({ where: { id: value, ownerId: u.id, isArchived: false } });
    if (!pet) return;
    await prisma.pet.update({ where: { id: pet.id }, data: { isArchived: true } });
    await ctx.reply("Питомец удалён. История записей сохранена.", { reply_markup: new InlineKeyboard().text("◀️ К питомцам", "b:pets") });
    return;
  }
  ...
  if (kind === "ok") {
    try {
      ...
      const booking = await createBooking({ clientId: u.id, petId: f.petId!, serviceId: f.serviceId!, masterId: f.masterId!, startsAt: studioDateTime(f.date!, f.minutes!, cfg.timezone) });
      await clearFlow(u.id);
      await ctx.reply(`✅ ЗАПИСЬ ПОДТВЕРЖДЕНА\n\n🐶 ${booking.pet.name}\n✂️ ${booking.service.name}\n👩 ${booking.master.name}\n📅 ${f.date}\n🕐 ${clock(f.minutes!)}\n💰 ${money(booking.priceKopecks, cfg.currency)}\n\n${cfg.bookingSuccessText}`, { reply_markup: main(isOwner(ctx)) });
      const text = `🆕 НОВАЯ ЗАПИСЬ\n\nКлиент: ${booking.client.firstName || "Клиент"}\nПитомец: ${booking.pet.name}\nУслуга: ${booking.service.name}\nМастер: ${booking.master.name}\nДата: ${f.date}\nВремя: ${clock(f.minutes!)}`;
      const actions = new InlineKeyboard().text("✅ Подтвердить", `a:confirm:${booking.id}`).text("❌ Отменить", `a:cancel:${booking.id}`);
      if (ownerId()) await bot.api.sendMessage(String(ownerId()), text, { reply_markup: actions });
    } catch (e) {
      await ctx.reply(e instanceof SlotTakenError ? "❌ К сожалению, это время уже заняли. Выберите другое." : "Не удалось создать запись. Попробуйте ещё раз.");
    }
    return;
  }
  if (kind === "x") {
    const b = await prisma.booking.findFirst({ where: { id: value, clientId: u.id, status: { in: ["PENDING", "CONFIRMED"] } } });
    if (b) { await prisma.booking.update({ where: { id: b.id }, data: { status: "CANCELLED" } }); await ctx.reply("❌ Запись отменена."); }
    return;
  }
  ...
  if (kind === "pe") {
    const pet = await prisma.pet.findFirst({ where: { id: value, ownerId: u.id, isArchived: false } });
    if (!pet) return;
    await setFlow(u.id, "pet_name", { editPetId: value, species: pet.species });
  ...
  if (kind === "back") {
    if (row?.state === "confirm") await chooseTime(ctx, u.id, f);
    else if (row?.state === "time") await chooseDate(ctx, u.id, f);
    else if (row?.state === "date") await chooseMaster(ctx, u.id, f);
    else if (row?.state === "master") await chooseService(ctx, u.id, f);
    else await choosePet(ctx, u.id);
  }
});

bot.callbackQuery(/^a:(confirm|cancel):(.+)$/, async ctx => {
  if (!isOwner(ctx)) {
    await ctx.answerCallbackQuery({ text: "Недостаточно прав", show_alert: true });
    return;
  }
  const [, action, id] = ctx.match;
  const booking = await prisma.booking.findFirst({ where: { id, status: { in: ["PENDING", "CONFIRMED"] } }, include: { client: true, pet: true, service: true, master: true } });
  if (!booking) { await ctx.answerCallbackQuery({ text: "Запись уже обработана" }); return; }
  const status = action === "confirm" ? "CONFIRMED" : "CANCELLED";
  await prisma.booking.update({ where: { id }, data: { status } });
  const cfg = await settings();
  await bot.api.sendMessage(
    String(booking.client.telegramId),
    action === "confirm"
      ? `✅ Ваша запись подтверждена\n\n${booking.service.name}\n${booking.master.name}\n${booking.startsAt.toLocaleString("ru-RU", { timeZone: cfg.timezone })}`
      : `❌ Ваша запись отменена администратором. Пожалуйста, выберите другое время.`
  );
  await ctx.answerCallbackQuery({ text: action === "confirm" ? "Запись подтверждена" : "Запись отменена" });
...
async function finishRegistration(ctx: Context, userId: string, pet?: { name: string; species: string; breed: string | null }) {
  ...
  await ctx.reply(
    `🎉 <b>Профиль создан!</b>\n\n👤 ${user.firstName || "—"}${pet ? `\n\n🐾 Питомец:\n${speciesLabel(pet.species)} ${pet.name}\n${pet.breed || "Порода не указана"}` : ""}\n\nТеперь вы можете записаться на груминг 💛`,
    { parse_mode: "HTML", reply_markup: main(isOwner(ctx)) }
  );
}
...
async function saveRegistrationPhone(ctx: Context, userId: string, phone: string) {
  ...
  await ctx.reply(
    "🐾 <b>Добавим вашего питомца?</b>",
    {
      parse_mode: "HTML",
      reply_markup: new InlineKeyboard()
        .text("🐶 Добавить собаку", "b:regpet:dog").row()
        .text("🐱 Добавить кошку", "b:regpet:cat").row()
        .text("⏭️ Пропустить", "b:regskip")
    }
  );
}
...
bot.on("message:contact", async ctx => {
  ...
  if (contact.user_id && contact.user_id !== ctx.from.id) return void ctx.reply("Пожалуйста, отправьте свой номер телефона.", { reply_markup: phoneKeyboard() });
  if (row?.state === "registration_phone") return void saveRegistrationPhone(ctx, u.id, contact.phone_number);
  if (row?.state === "profile_edit_phone") {
    const f = row.payload as Flow;
    await prisma.user.update({ where: { id: u.id }, data: { firstName: f.name, phone: contact.phone_number } });
    await clearFlow(u.id);
    await ctx.reply("✅ Данные обновлены.", { reply_markup: main(isOwner(ctx)) });
  }
});

bot.on("message:text", async ctx => {
  const u = await current(ctx), row = await prisma.botFlow.findUnique({ where: { userId: u.id } });
  if (!row) return;
  const f = row.payload as Flow, text = ctx.message.text.trim();

  // Registration flows
  if (row.state === "registration_name") {
    if (text.length < 2) return void ctx.reply("Введите, пожалуйста, имя.");
    await prisma.user.update({ where: { id: u.id }, data: { firstName: text } });
    await setFlow(u.id, "registration_phone");
    await ctx.reply("📱 Укажите номер телефона.", { reply_markup: phoneKeyboard() });
    return;
  }

  if (row.state === "registration_phone") {
    await saveRegistrationPhone(ctx, u.id, text);
    return;
  }

  if (row.state === "registration_pet_name") {
    if (!text) return;
    await setFlow(u.id, "registration_pet_breed", { ...f, name: text });
    await ctx.reply("🐕 Какая порода?\nМожно отправить «Не знаю».");
    return;
  }

  if (row.state === "registration_pet_breed") {
    const pet = { name: f.name!, species: f.species!, breed: ["не знаю", "—"].includes(text.toLowerCase()) ? null : text };
    await prisma.pet.create({ data: { ownerId: u.id, ...pet } });
    await finishRegistration(ctx, u.id, pet);
    return;
  }

  if (row.state === "profile_edit_name") {
    if (text.length < 2) return void ctx.reply("Введите, пожалуйста, имя.");
    await setFlow(u.id, "profile_edit_phone", { name: text });
    await ctx.reply("📱 Укажите новый номер телефона.", { reply_markup: phoneKeyboard() });
    return;
  }

  if (row.state === "profile_edit_phone") {
    if (text.length < 5) return void ctx.reply("Укажите корректный номер телефона.");
    await prisma.user.update({ where: { id: u.id }, data: { firstName: f.name, phone: text } });
    await clearFlow(u.id);
    await ctx.reply("✅ Данные обновлены.", { reply_markup: main(isOwner(ctx)) });
    return;
  }
  ...
  // Pet management
  if (row.state === "pet_name") {
    if (!text) return;
    await setFlow(u.id, "pet_species", { ...f, name: text });
    await ctx.reply("Выберите вид животного:", { reply_markup: new InlineKeyboard().text("🐶 Собака", "b:species:dog").text("🐱 Кошка", "b:species:cat") });
    return;
  }

  if (row.state === "pet_breed") {
    const petData = { name: f.name!, species: f.species!, breed: ["—", "не знаю"].includes(text.toLowerCase()) ? null : text };
    if (f.editPetId) await prisma.pet.update({ where: { id: f.editPetId }, data: petData });
    else await prisma.pet.create({ data: { ownerId: u.id, ...petData } });
    await clearFlow(u.id);
    await ctx.reply("🐾 Питомец сохранён.", { reply_markup: main(isOwner(ctx)) });
  }
});
