...
`npm run dev` запускает Next.js. Для тестирования бота без webhook отключите webhook в BotFather и отдельно выполните `npm run bot`.
...
Не используйте `prisma migrate reset` или `db push` для production базы. `npm run db:deploy` — сокращение для безопасного применения миграций.
...
`npm start` запускает Next.js и использует `PORT`, предоставленный Railway. HTTP проверка доступна по `GET /health`; при доступной БД она возвращает `{ "status": "ok" }`.
...
6. Railway применит `npm run db:deploy` до запуска, затем выполнит `npm run build` и `npm start` по [railway.json](railway.json).
7. На доверенном компьютере с этими же ENV выполните `npm run webhook:set`. В BotFather `/getWebhookInfo` должен показать URL `…/api/telegram/webhook` без ошибки.
...
10. Для reminders создайте в Railway отдельный **Cron Job** из этого же repository с командой `npm run reminders` и расписанием `*/5 * * * *`. Он использует ту же `DATABASE_URL` и bot token. Записи в `Notification` не позволят отправить напоминание повторно после рестарта.
...
- [x] `npm start` — production start command для Railway.
